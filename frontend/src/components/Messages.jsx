...
import {
  ...
  getCountOfMessages,
  addMessage,
  setInTheProcessSending,
  getInTheProcessSending,
...
const Messages = () => {
  ...
  const isInTheProcessSending = useSelector(getInTheProcessSending);
  ...
  const handleSubmitMessage = async (newMessage, userToken) => {
    try {
      ...
      dispatch(setInTheProcessSending());
    } catch (error) {
            ...
            <Form noValidate className="py-1 border rounded-2">
              <div className={values.message === '' ? 'input-group has-validation' : 'input-group'}>
                <Field
                  ...
                  // autoFocus
                />
                <Button type="submit" disabled={values.message === '' || isInTheProcessSending} className="btn btn-group-vertical">
